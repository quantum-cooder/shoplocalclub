...
class StampCardApi {
  static Future<List<StampCardUser>> getUsersStampCards({
    bool isArchived = false,
  }) async {
    final url = Uri.parse(
      isArchived
          ? ApiEndPoints.userStampCardsArchived
          : ApiEndPoints.userStampCards,
    );
    ...
    try {
      ...
      if (response.statusCode == ApiEndPoints.successCode) {
        ...
        if (data['result']) {
          ...
          return StampCardModel.instance?.data?.stampcardUsers ?? [];
        }
      }
    } catch (e) {
